...
"""
studenti = [
    {"nome": "Vito", "voti": [6, 8, 6, 5]},
    {"nome": "Alessio", "voti": [10, 7, 8]},
    {"nome": "Vincenzo", "voti": [8, 8, 6, 7]},
    {"nome": "Mauro", "voti": [5, 4, 3, 2]}
]

# devono essere tuple
medie_studenti = []

#il for va ad indentazione 
for studente in studenti:
    ...
    if len(voti) > 0:
        ...
        #sum somma elementi della lista (matematico)
        #len conta elementi che ci sono nella lista 
        
        media = sum(voti) / len(voti)
   
        #round per non avere troppi decimali
        medie_studenti.append((nome, round(media, 2)))
    else:
        medie_studenti.append((nome, 0.0))
...
#Dalla lista di tuple ottenuta, crea un set con i nomi degli studenti che hanno una media superiore o uguale a 6.
promossi = set()
for nome, media in medie_studenti: 
    if media >= 6:
        promossi.add(nome)



# 3. Dizionario con voti ordinati (usando la lista originale studenti)
voti_ordinati = {i["nome"]: sorted(i["voti"]) for i in studenti}

# 4. Stampa chiara
print(f"Lista di tuple (nome, media): {medie_studenti}")
print(f"Set di studenti con media >= 6: {promossi}")
print("Dizionario con voti ordinati:")
for nome, voti in voti_ordinati.items():
    print(f"{nome}: {voti}")
